...
unsigned char* UsuarioSolicitudDNS(DNS dns);

DNS CrearSolicitudDNS(unsigned char id,unsigned char* nombre,unsigned char tipo,unsigned char clase);
unsigned char* AnalizandoActualizacionDNS(DNS dns);
...
DNS EnviarSolicitud(unsigned char* nombre_dominio,Direccion dir_envio,Direccion dir_recibe){
    ...
    //como se tuvo exito al recibir el DNS lo analizamos y obtendremos
    //la ip de ese nombre de dominio
    unsigned char* IP=AnalizandoActualizacionDNS(dns_res);
...
unsigned char* UsuarioSolicitudDNS(DNS dns){
    ...
    printf("\n\n**************Fin Solicitud DNS del usuario**************\n");
    //Se imprime la trama que se va a enviar
    imprimeTrama(dns->array_dns,dns->longitud_array);

    return TramaToNombre(Nombre_Peticion);//se regresa la peticion del usuario
}
...
unsigned char* AnalizandoActualizacionDNS(DNS dns){
    ...
    unsigned char* IP4=(unsigned char*)malloc(sizeof(unsigned char)*4);
    ...
    printf("\n\n**************Fin actualizacion DNS del Proxy**************\n");
    imprimeTrama(dns->array_dns,dns->longitud_array);

    return IP4;
}

DNS CrearActualizacionDNS(unsigned char* IP_sust,DNS dns_sol){
    unsigned char temp[512];
    for (int i = 0; i < 512; i++){temp[i]=0x00;}
    ...
    // Estructura de Recursos RR 
    unsigned char Apuntador[]={0xc0,0x0c};//el nombre de dominio siemrpe estara ahi 
    unsigned char* Nombre_RR; // longitud variable, nombre del dominio que solicitamos
    unsigned char Tipo_Registro[]={0x00,0x01}; //logitud fija
    unsigned char Clase_Registro[]={0x00,0x01}; //logitud fija
    unsigned char RR_TTL[]={0x04,0x03,0x02,0x01}; //logitud fija, tiempo de vida en seg
    unsigned char Tam_Datos_RR[]={0x00,0x04}; //logitud fija, tamaño de la respuesta
    int num_saltos=0,ptr_aux=0,ptr=0;
    
    //copiando encabezado y entrada de solicitud
    memcpy(temp,dns_sol->array_dns,dns_sol->longitud_array);
    printf("\n****************Actualizacion DNS para el Cliente****************\n");
    printf("\n------------------Encabezado DNS----------------");
    //printf("\nptr=%d",ptr);
    memcpy(ID_Transaccion,temp+ptr,2);ptr+=2;
    printf("\n\tId de transaccion:%.2hx %.2hx", ID_Transaccion[0],ID_Transaccion[1]);
    ...
    //--------------creando la estructura de recursos-------------
    Nombre_RR=ArmaDominios(temp,ptr,&num_saltos);ptr+=num_saltos; //se regresa un numero de saltos que se tiene que dar
    memcpy(temp+ptr, Nombre_RR,strlen(Nombre_RR));ptr+=strlen(Nombre_RR);
    printf("\n\tNombre de RR:%s ",TramaToNombre(Nombre_RR));
    num_saltos=0;
    //printf("\nptr=%d",ptr);
    memcpy(temp+ptr,Tipo_Registro,2);ptr+=2;
    printf("\n\tTipo de Registro:%d ",unchar_to_int(Tipo_Registro));

    //printf("\nptr=%d",ptr);
    memcpy(temp+ptr,Clase_Registro,2);ptr+=2;
    printf("\n\tClase de Registro:%d ",unchar_to_int(Clase_Registro));
    
    //printf("\nptr=%d",ptr);
    memcpy(temp+ptr,RR_TTL,4);ptr+=4;
    printf("\n\tTiempo de respuesta: %d:%d:%d:%d Seg. ",RR_TTL[0],RR_TTL[1],RR_TTL[2],RR_TTL[3]);

    //printf("\nptr=%d",ptr);
    memcpy(temp+ptr,Tam_Datos_RR,2);ptr+=2;
    printf("\n\tTamaño de los Datos RR:%d ",unchar_to_int(Tam_Datos_RR));

    //printf("\nptr=%d",ptr);
    memcpy(temp+ptr,IP_sust,4);ptr+=4;
    printf("\n\tDatos RR:%d.%d.%d.%d ",IP_sust[0],IP_sust[1],IP_sust[2],IP_sust[3]);
    printf("\n\n**************Fin actualizacion DNS para el Cliente**************\n");


    DNS dns_res=NuevoDNS();
    dns_res->array_dns=(unsigned char*)malloc(sizeof(unsigned char)*(ptr));
    dns_res->longitud_array=ptr;
    return dns_res;
}
